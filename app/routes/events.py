...
def get_Events(db: Session = Depends(database.get_db), current_user: int = Depends(oauth2.get_current_user)):
...
def get_Events_by_me(db: Session = Depends(database.get_db), current_user: int = Depends(oauth2.get_current_user)):

    events = db.query(model.Event).filter(model.Event.user_id == current_user)
    if not events:
...
def create_event(event: schema.Events, db: Session = Depends(database.get_db), current_user: int = Depends(oauth2.get_current_user)):

    new_event = model.Event(user_id=current_user.id,content=event.content,description=event.description,
    area=event.area,region=event.region,pincode=event.pincode )
...
@router.get("/event/id", response_model=List[schema.Events])
...
def delete_event(id: int, db: Session = Depends(database.get_db), current_user: int = Depends(oauth2.get_current_user)):
...
def update_post(id: int, post: schema.Events, db: Session = Depends(database.get_db), current_user: int = Depends(oauth2.get_current_user)):
    ...
    event_query.update(post.dict(), synchronize_session=False)
    db.commit()

    return event_query.first()

@router.post("/respond/id", status_code=status.HTTP_201_CREATED)
def event_respond(EventRespond: schema.EventRespond, db: Session = Depends(database.get_db), current_user: int = Depends(oauth2.get_current_user)):
    ...
    if (EventRespond.dir == 1):
        if event_found:
            raise HTTPException(status_code=status.HTTP_409_CONFLICT,
                                detail=f'User with id:{current_user.id} already liked post with id:{EventRespond.event_id}')
        ...
        if not event_found:
            raise HTTPException(status_code=status.HTTP_409_CONFLICT,
                                detail=f'User with id:{current_user.id} has not liked post with id:{EventRespond.event_id}')
        event_query.delete(synchronize_session=False)
        db.commit()
