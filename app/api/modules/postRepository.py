...
def create_post(post:Post, db: Session,  user:int= Depends(get_current_user)):
    return create(post=post, db=db, user=user)

def myPost(db: Session, user: int = Depends(get_current_user)):
    return personal_post(db=db, user=user)

def get_all_post(db: Session, user: int = Depends(get_current_user)):
    return allPost(db=db)


def reactions(db: Session, like:Like, user:int=Depends(get_current_user)):
...
def delete_post(id: int, db: Session,  user : int = Depends(get_current_user)):
